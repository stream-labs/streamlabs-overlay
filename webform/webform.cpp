...
struct TWebf : public IUnknown
{
	...
	unsigned int
	    isnaving; // bitmask: 4=haven't yet finished Navigate call, 2=haven't yet received DocumentComplete, 1=haven't yet received BeforeNavigate
	//
...
TWebf::TWebf(HWND hhost)
{
	...
	HRESULT     hr;
	IOleObject* iole = 0;
	hr               = CoCreateInstance(CLSID_WebBrowser, 0, CLSCTX_INPROC_SERVER, IID_IOleObject, (void**)&iole);
	if (iole == 0)
		return;
...
{
	if (hf != INVALID_HANDLE_VALUE) {
		OutputDebugString(_T("Webform error: asked for Ready(), but something already is."));
		return hf;
	}
...
{
	if (hf == INVALID_HANDLE_VALUE) {
		OutputDebugString(_T("Webform error: tried to Set, but haven't Ready()ed"));
		return 0;
	}
	DWORD writ;
...
void TWebf::Go(const TCHAR* url)
{
	...
	if (url == 0) {
		if (hf == INVALID_HANDLE_VALUE) {
			OutputDebugString(_T("Webform error: tried to Go, but haven't yet Ready/Set"));
			return;
		}
		url = fn;
	}
...
IHTMLElement* TWebf::GetElement(const TCHAR* id)
{
	...
	if (hr == S_OK) {
		...
		if (hr == S_OK && disp != 0) {
			hr =
			    disp->QueryInterface(IID_IHTMLElement, (void**)&ret); // it's the caller's responsibility to release ret
			disp->Release();
		}
		doc_all->Release();
	}
